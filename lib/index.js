...
Patterns = require('./patterns.js')//Patterns for classes

//Запускает парсер js2css
function startStyle(object, units, alph, uses, patterns, mode){ 
	Units.setUnits(units);
	Alphabet.setRules(alph);
	Descriptor.setUse(uses);
	Patterns.setPatterns(patterns);
...
module.exports = function StyleSheet(def) {
	var units = Units.units,//defUnits
		...
		alphabet = {},
		uses = {},
		patterns = {};//extendable classes
	if (def && def.constructor.name==="Object") {
		rules = def;	
	}
	function startParse(mode) {//starter
		return startStyle(rules, units, alphabet, uses, patterns, mode)
	}
	return {
		...
		setPattern: function (name, pats) {
			if (name.constructor.name === "Object") {
				patterns = Object.assign({}, patterns, name);
				return this
			}
			patterns[name] = pats;
			return this
